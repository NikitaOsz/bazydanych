...
/*4. funkcja (zlicza ile noworodków urodziło się w danej sali.)*/
DELIMITER //
CREATE FUNCTION dziecko_sala(sala int)
	RETURNS INTEGER
BEGIN
	DECLARE ile INT;
	SELECT COUNT(*) INTO @ile FROM sale_porodowe_rezerwacja AS sale
	INNER JOIN pacjenci AS p ON sale.id_pacjenta = p.id_pacjenta
	INNER JOIN noworodki AS n ON p.id_pacjenta = n.id_pacjenta WHERE sale.nr_sali = sala;
    RETURN @ile;
END //
DELIMITER ;